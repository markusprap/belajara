...
RUMPUN_COMPETENCY_KEYS: dict[str, list[str]] = {
    ...
    # Specific programs
    "akuntansi": ["financial_accounting", "managerial_accounting", "auditing", "taxation", "accounting_information_systems"],
    "sistem informasi": ["software_engineering", "data_analytics", "enterprise_architecture", "quantitative_methods", "business_systems"],
    "manajemen": ["strategic_management", "finance_accounting", "marketing", "operations_logistics", "hr_organization"],
    "informatika": ["software_engineering", "data_ai", "system_architecture", "math_logic", "digital_business"],
    "hukum": ["hukum_dasar", "hukum_perdata", "hukum_pidana", "hukum_acara", "etika_profesi"],
    "farmasi": ["kimia_farmasi", "farmakologi", "farmasetika", "biologi_klinis", "praktik_profesi"],
    "kedokteran": ["ilmu_biomedik", "ilmu_klinis", "komunitas_pencegahan", "keterampilan_klinis", "etika_humaniora"],
    "psikologi": ["teori_psikologi", "asesmen_psikologi", "psikologi_klinis", "psikologi_terapan", "riset_etika"],
}
...
RUMPUN_COMPETENCY_LABELS: dict[str, dict[str, str]] = {
    ...
    "akuntansi": {
        "financial_accounting": "Akuntansi Keuangan",
        "managerial_accounting": "Akuntansi Manajemen",
        "auditing": "Auditing & Assurance",
        "taxation": "Perpajakan",
        "accounting_information_systems": "Sistem Informasi Akuntansi"
    },
    "sistem informasi": {
        "software_engineering": "Rekayasa Perangkat Lunak",
        "data_analytics": "Data & Analitik Bisnis",
        "enterprise_architecture": "Arsitektur Enterprise",
        "quantitative_methods": "Metode Kuantitatif",
        "business_systems": "Sistem Bisnis"
    },
    "manajemen": {
        "strategic_management": "Manajemen Strategis",
        "finance_accounting": "Keuangan & Akuntansi",
        "marketing": "Pemasaran",
        "operations_logistics": "Operasional & Logistik",
        "hr_organization": "SDM & Organisasi"
    },
    "informatika": {
        "software_engineering": "Rekayasa Perangkat Lunak",
        "data_ai": "Data Science & AI",
        "system_architecture": "Arsitektur Sistem",
        "math_logic": "Matematika & Logika",
        "digital_business": "Digital Business"
    },
    "hukum": {
        "hukum_dasar": "Hukum Dasar & Pengantar",
        "hukum_perdata": "Hukum Perdata & Bisnis",
        "hukum_pidana": "Hukum Pidana",
        "hukum_acara": "Hukum Acara & Praktik",
        "etika_profesi": "Etika & Profesi Hukum"
    },
    "farmasi": {
        "kimia_farmasi": "Kimia Farmasi",
        "farmakologi": "Farmakologi & Toksikologi",
        "farmasetika": "Farmasetika & Teknologi",
        "biologi_klinis": "Biologi & Ilmu Klinis",
        "praktik_profesi": "Praktik Profesi"
    },
    "kedokteran": {
        "ilmu_biomedik": "Ilmu Biomedik Dasar",
        "ilmu_klinis": "Ilmu Klinis",
        "komunitas_pencegahan": "Kedokteran Komunitas",
        "keterampilan_klinis": "Keterampilan Klinis",
        "etika_humaniora": "Etika & Humaniora"
    },
    "psikologi": {
        "teori_psikologi": "Teori Psikologi",
        "asesmen_psikologi": "Asesmen & Diagnostik",
        "psikologi_klinis": "Psikologi Klinis",
        "psikologi_terapan": "Psikologi Terapan",
        "riset_etika": "Riset & Etika Profesi"
    },
}
...
def infer_rumpun_from_prodi(study_program: str) -> str:
    ...
    # Specific prodi matching first
    if "akuntansi" in prodi_lower:
        return "akuntansi"
    if "sistem informasi" in prodi_lower:
        return "sistem informasi"
    if "informatika" in prodi_lower or "ilmu komputer" in prodi_lower or "computer science" in prodi_lower:
        return "informatika"
    if "manajemen" in prodi_lower:
        return "manajemen"
    if "hukum" in prodi_lower:
        return "hukum"
    if "farmasi" in prodi_lower:
        return "farmasi"
    if "kedokteran" in prodi_lower or "dokter" in prodi_lower:
        return "kedokteran"
    if "psikologi" in prodi_lower:
        return "psikologi"
...
def normalize_academic_profile(profile, study_program: str) -> dict:
    ...
    detected_prodi = profile.get("study_program") or study_program
    ...
    if not competency_scores:
        rumpun = infer_rumpun_from_prodi(detected_prodi)
    ...
    if not next_best_action:
        ...
        if gap_map["mandatory"]:
            ...
            next_best_action = f"Perkuat kompetensi inti program studi {detected_prodi} dan bangun portofolio untuk membuktikan keahlian Anda."

    return {
        "study_program": detected_prodi,
        ...
        "summary": str(profile.get("summary") or f"Analisis akademik untuk program studi {detected_prodi}."),
        ...
        "career_recommendations": [normalize_career(c, detected_prodi) for c in raw_careers],
...
def build_all_competency_dimensions_hint() -> str:
    """
    Returns a formatted string containing competency dimensions for all major study programs.
    """
    return """Berikut adalah daftar dimensi kompetensi berdasarkan Program Studi. Harap gunakan dimensi kompetensi yang sesuai dengan Program Studi yang terdeteksi dari dokumen:

=== Program Studi: HUKUM / ILMU HUKUM ===
...
=== Program Studi: KEDOKTERAN / DOKTER ===
...
=== Program Studi: INFORMATIKA / ILMU KOMPUTER / TEKNIK INFORMATIKA / COMPUTER SCIENCE ===
...
=== Program Studi: TEKNIK / ENGINEERING ===
...
=== Program Studi: PENDIDIKAN / KEGURUAN / PGSD / PAUD ===
...
=== Program Studi: KESEHATAN / KEPERAWATAN / KEBIDANAN / FISIOTERAPI / GIZI ===
...
def analyze_curriculum_text(
    ...
    all_competency_hints = build_all_competency_dimensions_hint()
...
TUGAS PENDETEKSIAN PROGRAM STUDI:
1. Cari tahu program studi (jurusan) mahasiswa dari dokumen transkrip akademik secara dinamis (misalnya mencari teks seperti "Sistem Informasi", "Akuntansi", "Informatika", dll.).
2. Jika berhasil mendeteksi nama program studi dari dokumen transkrip, gunakan itu sebagai Program Studi.
3. Jika tidak terdeteksi dari dokumen transkrip, gunakan default Program Studi masukan berikut: "{study_program}".

{all_competency_hints}
...
TUGAS ANALISIS:
1. Baca dokumen transkrip akademik dan identifikasi semua mata kuliah yang sudah diselesaikan (telah lulus dengan nilai kelulusan yang sah, biasanya grade C, C-, 2.0 ke atas, atau status lulus/lulus bersyarat). Jangan masukkan mata kuliah yang gagal/tidak lulus (seperti grade E, F, D di sebagian kampus, atau status tidak lulus), belum diambil, atau status mundur/withdrawn (W).
2. Tentukan nama program studi (jurusan) yang terdeteksi secara dinamis.
3. Berikan SKOR KOMPETENSI (0-100) untuk setiap 5 dimensi dari rumpun program studi yang terdeteksi tersebut.
4. Untuk setiap skor, berikan BUKTI KONKRIT: nama mata kuliah pendukung yang benar-benar ada di transkrip dan kutipan/petunjuk singkat dari teks dokumen.
5. Identifikasi GAP secara terstruktur: mandatory (wajib untuk prodi), elective (opsional tapi penting), career (untuk jalur karier tertentu).
6. Rekomendasikan maksimal 3 kelas dari katalog Belajara yang paling relevan untuk menutup gap.
7. Berikan NEXT BEST ACTION: 1 kalimat aksi paling penting yang harus dilakukan mahasiswa sekarang.
8. EKSTRAKSI MATA KULIAH SELESAI: Hanya ekstrak mata kuliah yang benar-benar ada di transkrip dengan grade lulus ke dalam "completed_subjects". JANGAN pernah memalsukan mata kuliah yang tidak ada di transkrip atau membuat-buat data mock/stub.

OUTPUT HARUS berupa JSON valid (tanpa markdown). Struktur persis:
{{
  "academic_profile": {{
    "study_program": "Nama program studi yang terdeteksi dari dokumen (misal: Akuntansi, Sistem Informasi, Informatika) atau fallback ke default jika tidak terdeteksi",
    "detected_semester": 4,
    "readiness_score": 72,
    "confidence_score": 80,
