  ...
  React.useEffect(() => {
    ...
    const initializeGoogleSignIn = () => {
      ...
      if (google?.accounts?.id) {
        // Clear interval as soon as SDK is found
          ...
          google.accounts.id.initialize({
            ...
            callback: async (response: any) => {
              ...
              try {
                ...
                if (payload) {
                  const res = await googleLogin(payload.email, payload.given_name || "", payload.family_name || "", payload.sub || "", formData.role, response.credential);
                  const user = res?.user;
        ...
        const btnContainer = document.getElementById("google-signup-button-target");
        if (btnContainer) {
    ...
    // Setup interval if SDK not yet available
    if (!(window as any).google?.accounts?.id) {
      intervalId = setInterval(initializeGoogleSignIn, 1000);
    }

    return () => {
      if (intervalId) clearInterval(intervalId);
    };
  }, [redirectUrl, router, googleLogin, formData.role]);
